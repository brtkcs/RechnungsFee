...
def auftrag_erstellen(data: "RechnungCreate", db: Session = Depends(get_db)):
    ...
    auftrag = Rechnung(
        ...
        partner_strasse=data.partner_strasse,
        partner_hausnummer=data.partner_hausnummer,
        partner_plz=data.partner_plz,
...
def forderungsausfall_buchen(rechnung_id: int, db: Session = Depends(get_db)):
    ...
    Reine Status-Änderung, keine Journalbuchung: Unter Ist-Versteuerung (§20 UStG, die
    einzige von RechnungsFee unterstützte Versteuerungsart) entsteht USt erst bei
    tatsächlichem Zahlungseingang – für den nie gezahlten Restbetrag wurde nie USt fällig,
    es gibt also nichts zu korrigieren (Issue #252)."""
    rechnung = db.query(Rechnung).filter(Rechnung.id == rechnung_id).first()
    if not rechnung:
        raise HTTPException(status_code=404, detail="Rechnung nicht gefunden.")
    if rechnung.ist_entwurf:
...
def _lieferschein_zu_rechnung_konvertieren(
    ...
    rechnung = Rechnung(
        ...
        partner_strasse=lieferscheine[0].partner_strasse,
        partner_hausnummer=lieferscheine[0].partner_hausnummer,
        partner_plz=lieferscheine[0].partner_plz,
...
def lieferschein_aus_rechnung(rechnung_id: int, db: Session = Depends(get_db)):
    ...
    ls = Rechnung(
        ...
        partner_strasse=r.partner_strasse,
        partner_hausnummer=r.partner_hausnummer,
        partner_plz=r.partner_plz,
...
def lieferschein_aus_angebot(angebot_id: int, db: Session = Depends(get_db)):
    ...
    ls = Rechnung(
        ...
        partner_strasse=angebot.partner_strasse,
        partner_hausnummer=angebot.partner_hausnummer,
        partner_plz=angebot.partner_plz,
...
def rechnung_aus_proforma(proforma_id: int, zahlung: ZahlungEingegangen, db: Session = Depends(get_db)):
    ...
    rechnung = Rechnung(
        ...
        partner_strasse=proforma.partner_strasse,
        partner_hausnummer=proforma.partner_hausnummer,
        partner_plz=proforma.partner_plz,
...
def rechnung_aus_auftrag(auftrag_id: int, db: Session = Depends(get_db)):
    ...
    rechnung = Rechnung(
        ...
        partner_strasse=auftrag.partner_strasse,
        partner_hausnummer=auftrag.partner_hausnummer,
        partner_plz=auftrag.partner_plz,
