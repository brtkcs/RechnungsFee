    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 space-y-5 flex-1 overflow-y-auto">
        ...
        {auftrag.positionen?.length > 0 && (
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Positionen</p>
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
              <table className="w-full text-xs">
                <tbody>
                  {auftrag.positionen.map((p, i) => (
                    <tr key={i} className={i > 0 ? 'border-t border-slate-100 dark:border-slate-700' : ''}>
                      <td className="px-3 py-2 text-slate-700 dark:text-slate-200">{p.beschreibung}</td>
                      <td className="px-3 py-2 text-right text-slate-500 dark:text-slate-400 w-16">{p.menge} {p.einheit}</td>
                      <td className="px-3 py-2 text-right text-slate-700 dark:text-slate-200 w-24">
                        {(parseFloat(p.brutto as any) * parseFloat(p.menge as any)).toFixed(2).replace('.', ',')} €
                      </td>
                    </tr>
                  ))}
                </tbody>
